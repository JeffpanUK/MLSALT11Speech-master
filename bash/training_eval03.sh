...
'''
for model in grph-plp; do
	for ad in plp; do #grph-plp adaptation is removed for error output
...
tgdir="cross-adapt/plp-adapt-grph-plp" 
passdir="adapt" 
outdir='LOG.align' 
while read line
do
...
for model in plp grph-plp tandem grph-tandem hybrid; do
	for ad in plp tandem grph-tandem; do
		for de in decode decode_cn;do
			tgs="${ad}-adapt-${model}"
			echo $tgs >> ${Logdir}
			echo | ./scripts/score.sh ./cross-adapt/${tgs} eval03 ${de} >> ${Logdir}
		done
	done
done
...
Logdir="./challenge/eval03.individual"
echo 'scoring the adapted systems'
for model in plp grph-plp tandem grph-tandem hybrid; do
	for de in decode decode_cn;do
		tgs=${model}
...
8. ROVER combination
'''
Logdir="./challenge/eval03.ROVER"
tg1='hybrid'
tg2='plp-adapt-grph-plp'
tgs="${tg1}+${tg2}_${t}"
echo $tgs
while read line
do
...
python exp/shell/RoverCombi.py ${tg1} ${tg2} ${t} ${testlist}/eval03.lst --pass1 . --pass2 cross-adapt
echo $tgs >> ${Logdir}
echo | ./scripts/score.sh ./mlf_combine/${tgs}/combine eval03 ${t} >> ${Logdir}
...
# 							mkdir -p ./mlf_combine/${tgs}/combine/${line}/${t}/
# 						done < ${testlist}/eval03.lst
# 						python exp/shell/RoverCombi.py ${tg1} ${tg2} ${t} ${testlist}/eval03.lst \
...
# 					done < ${testlist}/eval03.lst
# 					python exp/shell/CNC.py ${tg1} ${tg2} ${t} ${testlist}/eval03.lst --pass1 . --pass2 cross-adapt
# 					echo $tgs >> ${Logdir}
# 					echo | ./scripts/score.sh ./mlf_cnc/${tgs}/combine eval03 ${t} >> ${Logdir}
# 				fi
# 				rm -r ./mlf_cnc/${tgs}/
# 			done
# 		done
...
# 							mkdir -p ./mlf_cnc/${tgs}/combine/${line}/${t}/
# 						done < ${testlist}/eval03.lst
# 						python exp/shell/CNC.py ${tg1} ${tg2} ${t} ${testlist}/eval03.lst --pass1 cross-adapt --pass2 cross-adapt
# 						echo $tgs >> ${Logdir}
# 						echo | ./scripts/score.sh ./mlf_cnc/${tgs}/combine eval03 ${t} >> ${Logdir}
# 					fi
# 					rm -r ./mlf_cnc/${tgs}/
...
# 	Logdir="./challenge/eval03.${type}"
# 	savepath="./challenge/sorted_eval03.${type}"
